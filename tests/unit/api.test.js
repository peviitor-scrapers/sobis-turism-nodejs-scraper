...
describe('scraper/api.js', () => {
  ...
  describe('querySOLR', () => {
    ...
    it('should throw on HTTP error', async () => {
      ...
      await expect(solr.querySOLR('794572')).rejects.toThrow('API jobs query error: 500');
    });
  });
  ...
  describe('upsertJobs', () => {
    ...
    it('should skip API call when jobs array is empty', async () => {
      await expect(solr.upsertJobs([])).resolves.not.toThrow();
      expect(mockFetch).not.toHaveBeenCalled();
    });

    it('should skip API call when jobs is null/undefined', async () => {
      await expect(solr.upsertJobs(null)).resolves.not.toThrow();
      await expect(solr.upsertJobs(undefined)).resolves.not.toThrow();
      expect(mockFetch).not.toHaveBeenCalled();
    });
  ...
  describe('deleteJobsByCIF', () => {
    ...
    it('should throw on HTTP error', async () => {
      ...
      await expect(solr.deleteJobsByCIF('794572')).rejects.toThrow('API jobs delete error: 500');
    });
  });
